...
export class AdaService extends Service {
  ...
  async craftStakeTx(
    ...
    try {
      ...
      if (!address.stake_address) {
        throw new Error('Could not fetch stake address');
      }

      const stakeKeyHash = await this.getStakeKeyHash(address.stake_address);
      if (!stakeKeyHash) {
        throw new Error('Could not hash stake key');
      }
      ...
      const stake: TaggedStake = {
        stakeAddress: address.stake_address,
        balance: Number(outAmount),
      };
  ...
  async craftWithdrawRewardsTx(
    ...
    try {
      ...
      const address = await this.client.addresses(walletAddress);
      if (!address.stake_address) {
        throw new Error('No stake address');
      }
      ...
      if (!rewardAddress) {
        throw new Error('Could not retrieve rewards address');
      }
   ...
   */
  async getRewardsByAccounts(accountIds: string[]): Promise<AdaRewards> {
    const { data } = await api.get<AdaRewards>(
      `/v1/ada/rewards?accounts=${accountIds.join(',')}`,
    );
    return data;
  }
   ...
   * @returns {AdaRewards} Cardano rewards
   */
  async getRewardsByStakeAddresses(stakeAddresses: string[]): Promise<AdaRewards> {
    const { data } = await api.get<AdaRewards>(
      `/v1/ada/rewards?stake_addresses=${stakeAddresses.join(',')}`,
    );
    return data;
  }
   ...
   * @returns {AdaRewards} Cardano rewards
   */
  async getRewardsByWallets(wallets: string[]): Promise<AdaRewards> {
    const { data } = await api.get<AdaRewards>(
      `/v1/ada/rewards?wallets=${wallets.join(',')}`,
    );
    return data;
  }
}
