import { ksmToPlanck, Kiln } from '../src/kiln.ts';
...
const txRequest = await k.client.POST('/ksm/transaction/bond-extra-pool', {
  body: {
    account_id: kilnAccountId,
    member_account: fireblocksWallet,
    amount_planck: ksmToPlanck('0.1').toString(),
  },
});
if (txRequest.error) {
...
const signRequest = await (async () => {
  try {
    return await k.fireblocks.signKsmTx(vault, txRequest.data.data);
  } catch (err) {
...
const broadcastedRequest = await k.client.POST('/ksm/transaction/broadcast', {
  body: {
